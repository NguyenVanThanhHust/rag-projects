...
load_dotenv()
COLLECTION_NAME = os.getenv("COLLECTION_NAME", "MILVUS_DOCS")
MILVUS_ENDPOINT = os.getenv("MILVUS_ENDPOINT", "127.0.0.1:9000")
...
dim = len(emb_text(embedding_model=embedding_model, text="test"))

create_collection(milvus_client, collection_name=COLLECTION_NAME, dim=dim)
data_dir = sys.argv[-1]
...
for i, filepath in enumerate(tqdm(text_dict, desc="Creating embeddings")):
    ...
    for line in chunks:
        try:
            vector = emb_text(embedding_model=embedding_model, text=line)
